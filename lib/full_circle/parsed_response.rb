  ...
  class ParsedResponse
    ...
    def build_metadata
      ResponseMetadata.new(
        page: response_hash.fetch("page"){1},
        total_pages: response_hash.fetch("totalPages"){1},
        total_results: response_hash.fetch("totalResults"){results.length},
        results_per_page: response_hash.fetch("resultsPerPage"){results.length}
      )
    end
